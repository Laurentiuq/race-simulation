...
public class CarAI : MonoBehaviour
{
    ...
    private void PathProgress() //Checks if the agent has reached the currentWayPoint or not. If yes, it will assign the next waypoint as the currentWayPoint depending on the input
    {
        ...
        void CreatePath()
        {
            if (CustomDestination == null && CustomDestinations.Count == 0)
            {
                if (Patrol == true)
                    RandomPath();
                else
            ...
            else
            {
                ...
                {
                    for (int i = 0; i < CustomDestinations.Count; i++)
                    {
                        if (CustomDestinations[i] != null)
                        {
                            CustomPath(CustomDestinations[i]);
                            // break;
                        }
                        else
                        {
                            debug("Custom Destination is null. Skipping to the next destination", false);
                            CustomDestinations.RemoveAt(i);
                        }
                    }
                }
                else
